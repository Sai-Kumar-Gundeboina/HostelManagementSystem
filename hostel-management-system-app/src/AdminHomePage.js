...
export default function AdminHomePage() {
  ...
  const handleApprove= async (id)=>{
    ...
    try{
      ...
      const token = localStorage.getItem('token');
      await axios.post(`http://localhost:1880/approve-tenant/${id}`, {
      roomNo: roomNumber
      }, {
        headers: {
          Authorization: `Bearer ${token}`
      ...
      <tbody>
        {tenantData.map((tenant) => (
          <tr key={tenant.id}>
            ...
            <td ><strong>{tenant.allocatedRoom}</strong></td>
            { localStorage.getItem('role') === "admin"?(
                <>
                    <td><button onClick={()=>handleApprove(tenant.id)}>Approve</button></td>
                </>
            ):null}
